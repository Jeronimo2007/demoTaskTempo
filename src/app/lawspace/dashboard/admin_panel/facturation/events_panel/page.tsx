...
import axios from 'axios'; // Removed AxiosError import as it's unused
import { useAuthStore } from '@/store/useAuthStore'; // Import the auth store
import { getAuthHeaders } from '@/app/utils/auth-utils'; // Removed AuthHeaders as it's unused
...
const EventsPanel = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      const response = await axios.get(`${API_URL}/users/get_all_users`, getAuthHeaders());
  ...
  useEffect(() => {
    if (authStore.token) {
      ...
      message.warning('No hay token de autenticación disponible al iniciar');
    }
  }, [authStore.token, fetchEvents, fetchUsers]); // Ensured dependencies are included
  ...
  const handleUpdateEvent = async (values: EventFormData) => {
    if (!editingEvent) {
      message.error('No hay evento seleccionado para editar');
      return;
    }
  ...
  const handleDeleteEvent = async (id: number) => {
    ...
    try {
      await axios.delete(`${API_URL}/events/delete/${id}`, getAuthHeaders());
